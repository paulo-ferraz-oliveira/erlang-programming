...
initialize([]) -> [];
initialize([{Handler, InitData} | Rest]) -> [{Handler, Handler:init(InitData)} | initialize(Rest)].
...
terminate([]) -> [];
terminate([{Handler, Data} | Rest]) -> [{Handler, Handler:terminate(Data)} | terminate(Rest)].
...
handle_msg({delete_handler, Handler}, LoopData) ->
    case lists:keysearch(Handler, 1, LoopData) of
        ...
        {value, {Handler, Data}} -> Reply = {data, Handler:terminate(Data)},
...
swap_handlers(Name, OldHandler, NewHandler) ->
    Data = delete_handler(Name, OldHandler),
    NewHandler:init(OldHandler:terminate(Data)),
    terminate({OldHandler, Data}),
    add_handler(Name, NewHandler, []).